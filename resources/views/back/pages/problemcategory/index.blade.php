...
@section('content')
    <div id="page-wrapper">
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header">Manage Problems</h1>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-12">
                            ...
                            <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($problemcategories as $problemcategory)
                                        ...
                                        <td>{{ $problemcategory->problemType->name }}</td>
                                        <td>{{ $problemcategory->name }}</td>
                                        <td>{{ $problemcategory->points }}</td>
                                        <td>
                                            @if ($problemcategory->status)
                                        ...
                                        <td>
                                            <a href="{{ route('problemcategory.edit', $problemcategory->id) }}">
                                            ...
                                            <span style="display:inline-block">
                                                <form action="{{ route('problemcategory.destroy', $problemcategory->id) }}"
                                                    method="post">
                                                    {{ csrf_field() }}
                                                    {{ method_field('DELETE') }}
